...
import { ProviderRegistryPrismaService } from '../interfaces/provider-registry.interface';
import { IntegrationEventLoggerService } from '../services/integration-event-logger.service';

// Router PrismaService combines both services
...
export class IntegrationRouter {
  ...
  constructor(
    @Optional() private readonly eventLogger?: IntegrationEventLoggerService,
    @Optional() private readonly prisma?: RouterPrismaService
  ) {}
  ...
  async execute<T = BasePayload>(
    ...
    for (const p of ranked) {
      ...
      try {
        ...
        const latencyMs = Date.now() - startTime;
        lastError = e instanceof Error ? e : e ? new Error(String(e)) : null;

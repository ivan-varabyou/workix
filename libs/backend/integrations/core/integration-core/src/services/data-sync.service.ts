...
export class DataSyncService {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        const items = Array.isArray(response.data)
          ? response.data.filter((item): item is BasePayload => isBasePayload(item))
          : [];
  ...
  async pushData(
    ...
    try {
      const results: Array<IntegrationResponse<BasePayload>> = [];

      for (const item of items) {
        // Convert generic format back to provider-specific format
        const payload = this.convertToProviderFormat(item, providerId);
   ...
   */
  private convertToProviderFormat(data: GenericDataFormat, _providerId: string): BasePayload {

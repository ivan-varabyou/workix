...
import {
  ...
  isBasePayload,
  RequestParams,
} from '../../../src/interfaces/integration-payload.interface';
import { AdapterEndpoint } from '../interfaces/adapter-config.interface';
import {
...
export class AdapterBuilderService {
  ...
  private buildAdapter(config: AdapterConfig): IntegrationProvider {
    ...
    const adapter: IntegrationProvider = {
      ...
      execute: async <T = any>(request: IntegrationRequest): Promise<IntegrationResponse<T>> => {
  ...
  private async executeRequest<T = BasePayload>(
    ...
    const endpoint = config.endpoints[request.operation];
    if (!endpoint) {
      throw new Error(
    ...
    const params = this.buildQueryParams(endpoint, request.params);
      ...
      switch (endpoint.method) {
        ...
        case 'POST':
          response = await firstValueFrom(
            this.httpService.post<T>(url, request.data || {}, { headers, params })
          );
          break;
        case 'PUT':
          response = await firstValueFrom(
            this.httpService.put<T>(url, request.data || {}, { headers, params })
          );
          break;
        case 'DELETE':
        ...
        case 'PATCH':
          response = await firstValueFrom(
            this.httpService.patch<T>(url, request.data || {}, { headers, params })
          );
          break;
        default:
          throw new Error(
   ...
   */
  private buildHeaders(config: AdapterConfig, endpoint: AdapterEndpoint): Record<string, string> {
  ...
  private getNestedValue(
    ...
    // Return type is correct, but we need to ensure the return value matches

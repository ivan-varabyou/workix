...
export class VirtualWorkerController {
  ...
  async listWorkers(
    ...
    const workerType: WorkerType | undefined =
      type && isValidWorkerType(type) ? type : undefined;
  ...
  async getTask(@Param('workerId') workerId: string, @Param('taskId') taskId: string): Promise<Task> {

    ...
    if (!isProduction) {
      ...
      try {
        const config: ReturnType<DocumentBuilder['build']> = new DocumentBuilder()
          .setTitle('Workix Auth Service')
          .setDescription(`Authentication Microservice for Workix Platform
...
- Password strength validation`)
          ...
          .addServer(`http://localhost:${process.env.API_AUTH_PORT || 7102}/${globalPrefix}`, 'Development server')
          .addBearerAuth({
            type: 'http',
            scheme: 'bearer',
            bearerFormat: 'JWT',
        ...
        SwaggerModule.setup('docs', app, document, {
          ...
          customSiteTitle: 'Workix Auth API',
          customCss: `
        ...
        Logger.log(`🌐 Swagger UI available at: http://localhost:${process.env.API_AUTH_PORT || 7102}/docs`);
      } catch (error: unknown) {
    ...
    const port: string | number = process.env.API_AUTH_PORT || 7102;

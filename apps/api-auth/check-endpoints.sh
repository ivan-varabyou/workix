...
PORT=${API_AUTH_PORT:-7102}
BASE_URL="http://localhost:${PORT}/api-auth/v1"
...
if curl -s -f "${HEALTH_URL}" > /dev/null 2>&1; then
    ...
    echo "   Убедитесь, что сервис запущен: nx serve api-auth"
    exit 1
fi
...
echo "   - Интеграционные тесты: nx test api-auth --testPathPattern=integration"

...
async function bootstrap(): Promise<void> {
  try {
    Logger.log('Starting Workflows Service bootstrap...');
    const startTime: number = Date.now();
    const app: INestApplication = await NestFactory.create(AppModule, {
      logger: ['error', 'warn', 'log', 'debug', 'verbose'],
    });
    const createTime: number = Date.now() - startTime;
    Logger.log(`✅ Workflows Service created successfully in ${createTime}ms`);
    ...
    if (!isProduction) {
      const config = new DocumentBuilder()
        .setTitle('Workflows Service API')
        .setDescription('API for managing workflows and workflow events')
        .setVersion('1.0')
        .addBearerAuth()
        .build();
    ...
    const port: number = configService.get<number>('PORT') || 7106;
    await app.listen(port);
    Logger.log(`✅ Workflows Service is running on: http://localhost:${port}`);
    if (!isProduction) {
      Logger.log(`📚 Swagger UI: http://localhost:${port}/docs`);
    }
  } catch (error: unknown) {
    const errorMessage: string = error instanceof Error ? error.message : String(error);
    Logger.error(`Failed to start Workflows Service: ${errorMessage}`, error instanceof Error ? error.stack : String(error));
    process.exit(1);
  }
}

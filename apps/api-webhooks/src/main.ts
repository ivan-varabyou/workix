...
async function bootstrap(): Promise<void> {
  try {
    Logger.log('Starting Webhooks Service bootstrap...');
    const startTime: number = Date.now();
    const app: INestApplication = await NestFactory.create(AppModule, {
      logger: ['error', 'warn', 'log', 'debug', 'verbose'],
    });
    const createTime: number = Date.now() - startTime;
    Logger.log(`✅ Webhooks Service created successfully in ${createTime}ms`);
    ...
    if (!isProduction) {
      const config = new DocumentBuilder()
        .setTitle('Webhooks Service API')
        .setDescription('API for managing webhooks and webhook events')
        .setVersion('1.0')
        .addBearerAuth()
        .build();
    ...
    const port: number = configService.get<number>('PORT') || 7105;
    await app.listen(port);
    Logger.log(`✅ Webhooks Service is running on: http://localhost:${port}`);
    if (!isProduction) {
      Logger.log(`📚 Swagger UI: http://localhost:${port}/docs`);
    }
  } catch (error: unknown) {
    const errorMessage: string = error instanceof Error ? error.message : String(error);
    Logger.error(`Failed to start Webhooks Service: ${errorMessage}`, error instanceof Error ? error.stack : String(error));
    process.exit(1);
  }
}

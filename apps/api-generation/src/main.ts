...
async function bootstrap(): Promise<void> {
  try {
    Logger.log('Starting Generation Service bootstrap...');
    const startTime: number = Date.now();
    const app: INestApplication = await NestFactory.create(AppModule, {
      logger: ['error', 'warn', 'log', 'debug', 'verbose'],
    });
    const createTime: number = Date.now() - startTime;
    Logger.log(`✅ Generation Service created successfully in ${createTime}ms`);
    ...
    if (!isProduction) {
      const config = new DocumentBuilder()
        .setTitle('Generation Service API')
        .setDescription('API for AI-powered content generation (text, image, video, speech)')
        .setVersion('1.0')
        .addBearerAuth()
        .build();
    ...
    const port: number = configService.get<number>('PORT') || 7111;
    await app.listen(port);
    Logger.log(`✅ Generation Service is running on: http://localhost:${port}`);
    if (!isProduction) {
      Logger.log(`📚 Swagger UI: http://localhost:${port}/docs`);
    }
  } catch (error: unknown) {
    const errorMessage: string = error instanceof Error ? error.message : String(error);
    Logger.error(`Failed to start Generation Service: ${errorMessage}`, error instanceof Error ? error.stack : String(error));
    process.exit(1);
  }
}

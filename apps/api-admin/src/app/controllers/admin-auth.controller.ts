...
import { Admin2FAIntegrationService } from '../services/admin-2fa-integration.service';
import { AdminPasswordResetService } from '../services/admin-password-reset.service';

/**
 * Auth Controller
 * Handles authentication and registration for admin API
 */
@ApiTags('auth')
@Controller('auth')
export class AuthController {
  constructor(
    ...
    private admin2FAIntegrationService: Admin2FAIntegrationService,
  ...
  async verify2FA(@Body() dto: Verify2FADto, @Req() req: AdminRequest): Promise<AdminTokensResponse & { admin: { id: string; email: string; role: string } }> {
    ...
    return await this.admin2FAIntegrationService.verify2FAAndLogin(adminId, dto.totpCode);
  }

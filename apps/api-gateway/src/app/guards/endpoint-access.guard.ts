...
export class EndpointAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const appId: string = request.headers['x-application-id'] as string;
    ...
    const service = this.detectServiceFromPath(path);
  ...
  private detectServiceFromPath(path: string): string | null {
    const cleanPath = path.replace(/^\/api\/v\d+\//, '/api/');

import { Injectable, UnauthorizedException, ConflictException, BadRequestException, NotFoundException, Logger, Inject, Optional } from '@nestjs/common';
import * as bcrypt from 'bcrypt';

import { GatewayPrismaService } from '../../prisma/gateway-prisma.service';

import { AdminJwtService, AdminTokensResponse, AdminIpWhitelistService, AdminTokenCacheService } from '@workix/domain/admin';
...
export class AdminAuthService {
  ...
  async register(dto: AdminRegisterDto, createdBy?: string): Promise<{ id: string; email: string; role: string }> {
    ...
    const existing = await this.prisma.gatewayAdmin.findUnique({
    ...
    const admin = await this.prisma.gatewayAdmin.create({
      data: createData,
    });
  ...
  async login(dto: AdminLoginDto, ipAddress?: string, userAgent?: string): Promise<AdminTokensResponse & { admin: { id: string; email: string; role: string } }> {
    ...
    if (!isValid) {
      const loginDuration = (Date.now() - loginStartTime) / 1000;
  ...
  async logout(token: string, adminId: string): Promise<void> {
    ...
    const sessions = await this.prisma.gatewayAdminSession.findMany({
      where: { adminId },
      select: {
        id: true,
        tokenHash: true,
  ...
  async revokeSession(adminId: string, sessionId: string, currentToken: string): Promise<void> {
    // Find session
    const session = await this.prisma.gatewayAdminSession.findFirst({
      where: {
        id: sessionId,
        adminId,
      },
  ...
  async revokeAllSessions(adminId: string, currentToken: string): Promise<{ revokedCount: number }> {
    ...
    const deleteResult = await this.prisma.gatewayAdminSession.deleteMany({
      where: {
        adminId,
  ...
  async getLoginHistory(
    ...
    // Get total count
    const total = await this.prisma.gatewayAuditLog.count({ where });

    // Get entries
    const logs = await this.prisma.gatewayAuditLog.findMany({
      where,
      orderBy: { createdAt: 'desc' },
      skip,
      take: limit,

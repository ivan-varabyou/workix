 ...
 * Handles password reset requests and token verification for Gateway admins
 */
@Injectable()
export class AdminPasswordResetService {
  ...
  async requestPasswordReset(email: string, ipAddress?: string, userAgent?: string): Promise<{ message: string }> {
    ...
    const recentResets = await this.prisma.gatewayPasswordReset.count({
      where: {
        adminId: admin.id,
    ...
    await this.prisma.gatewayPasswordReset.updateMany({
      where: {
        adminId: admin.id,
        usedAt: null,
      },
    ...
    await this.prisma.gatewayPasswordReset.create({
      data: resetData,
    });
  ...
  async verifyResetToken(token: string): Promise<{ adminId: string; email: string }> {
    const passwordReset = await this.prisma.gatewayPasswordReset.findUnique({
      where: { token },
      include: { admin: true },
    });
  ...
  async resetPassword(token: string, newPassword: string, ipAddress?: string, userAgent?: string): Promise<{ message: string }> {
    ...
    // Mark token as used
    await this.prisma.gatewayPasswordReset.update({

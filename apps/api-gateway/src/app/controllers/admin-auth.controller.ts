import { Body, Controller, Delete, Get, Param, Post, Query, HttpCode, HttpStatus, UseGuards, Request, SetMetadata, Req, ForbiddenException, Inject, Optional } from '@nestjs/common';
...
export class AdminAuthController {
  ...
  async register(@Body() dto: AdminRegisterDto, @Request() req: any): Promise<{ id: string; email: string; role: string }> {
    return await this.adminAuthService.register(dto, req.admin?.id);
  }
  ...
  private extractIpAddress(req: AdminRequest): string {
    ...
    if (Array.isArray(forwardedFor) && forwardedFor.length > 0) {
      const firstElement = forwardedFor[0];
      if (typeof firstElement === 'string' && firstElement) {
        const firstIp = firstElement.split(',')[0];
        if (firstIp) {
          return firstIp.trim();
        }
      }
  ...
  async revokeAllSessions(@Req() req: AdminRequest): Promise<{ message: string; revokedCount: number }> {
    ...
    const result = await this.adminAuthService.revokeAllSessions(adminId, currentToken);
    return {
  ...
  async logout(@Request() req: any) {
    await this.adminAuthService.logout(req.adminToken, req.admin.id);
  ...
  async getIpWhitelist(@Request() req: any) {
    return this.ipWhitelistService.getWhitelistedIps(req.admin.id);
  }
  ...
  async addIpToWhitelist(
    ...
    return this.ipWhitelistService.addIp(req.admin.id, dto.ipAddress, dto.description);
  }
  ...
  async removeIpFromWhitelist(
    ...
    await this.ipWhitelistService.removeIp(req.admin.id, ipId);
  ...
  async toggleIpWhitelist(
    ...
    await this.ipWhitelistService.setIpWhitelistEnabled(req.admin.id, dto.enabled);
  ...
  async healthCheck(): Promise<AdminHealthCheckResponseDto> {
    ...
    if (this.cacheManager) {
      try {
        ...
        await (this.cacheManager as any).get('health_check');
    ...
    // Check JWT secret
    try {
      const jwtSecret = this.configService.get<string>('GATEWAY_ADMIN_JWT_SECRET') ||
    ...
    const hasWarnings = checks.redis.status === 'not_configured';

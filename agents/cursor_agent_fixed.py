"""
Cursor IDE Agent с использованием Ollama (ИСПРАВЛЕННАЯ ВЕРСИЯ)
Использует llama3.1:8b-instruct-q4_K_M для работы в IDE
Исправлена ошибка 400: Invalid request body
"""
...
# ИСПРАВЛЕННАЯ конфигурация LLM для Ollama
# Использует config_list формат для совместимости с AutoGen
OLLAMA_CONFIG = {
    "config_list": [{
        "model": os.getenv("OLLAMA_MODEL", "llama3.1:8b-instruct-q4_K_M"),
        "base_url": os.getenv("OLLAMA_BASE_URL", "http://localhost:11434/v1"),
        "api_key": "ollama",  # Может быть любая строка
        "api_type": "open_ai",
    }],
...
if __name__ == "__main__":
    print("🚀 Cursor IDE Agent запущен! (ИСПРАВЛЕННАЯ ВЕРСИЯ)")
    print(f"📦 Модель: {OLLAMA_CONFIG['config_list'][0]['model']}")
    ...
    print("\n💡 Если возникает ошибка 400, используйте эту версию (cursor_agent_fixed.py)")
